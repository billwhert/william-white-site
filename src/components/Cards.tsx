...
export function LinkCard({ title, href, note, imageUrl }: { title: string; href: string; note?: string; imageUrl?: string }) {
  // ZERO URL parsing to avoid runtime throws
  const isExternal = /^https?:\/\//i.test(href)
  const favicon = imageUrl || (isExternal ? 'https://www.google.com/s2/favicons?domain=example.com&sz=64' : 'assets/favicon.svg')
  return (
    <a href={href} target={isExternal ? '_blank' : '_self'} rel="noopener noreferrer" className="group flex items-center gap-3 rounded-2xl bg-white/90 border border-slate-200 p-4 hover:shadow-lg hover:-translate-y-0.5 transition">
      <img src={favicon} alt="" className="w-10 h-10 rounded" />
      <div>
        <div className="font-medium text-slate-900 group-hover:underline">{title}</div>
        <div className="text-xs text-slate-500">{isExternal ? 'external' : 'local'}{note ? ' • ' + note : ''}</div>
      </div>
    </a>
  )
}
...
export function BadgeCard({ title, href, issuer, note }: { title: string; href: string; issuer: string; note?: string }) {
  const isExternal = /^https?:\/\//i.test(href)
  const favicon = isExternal ? 'https://www.google.com/s2/favicons?domain=example.com&sz=64' : 'assets/favicon.svg'
  return (
